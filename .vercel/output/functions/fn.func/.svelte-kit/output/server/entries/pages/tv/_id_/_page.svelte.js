...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(Header, "Header").$$render(
    ...
    {},
    {}
  )} <div class="container"><div class="overview"><h1 class="align-center p-1" data-svelte-h="svelte-4717y0">Description</h1> <p class="p-1">${escape(data.movie.overview)}</p></div> <br> <br> <div class="container"><h1 class="align-center py-2" data-svelte-h="svelte-ed5c98">Watch links</h1> <div class="links carousel"><a class="btn btn-dark"${add_attribute("href", `https://vidsrc.to/embed/tv/${data.movie.id}`, 0)} target="_blank"><i class="fa-solid fa-play pr-1"></i>
                vidsrc</a> <a class="btn btn-dark"${add_attribute("href", `https://multiembed.mov/?video_id=${data.movie.id}&tmdb=1&s=1&e=1`, 0)} target="_blank"><i class="fa-solid fa-play pr-1"></i>
                superEmbed</a> <a class="btn btn-dark"${add_attribute("href", `https://seez.su/tv/${data.movie.id}`, 0)} target="_blank"><i class="fa-solid fa-play pr-1"></i>
                seez.su</a> <a class="btn btn-dark"${add_attribute("href", `https://vidsrc.me/embed/tv?tmdb=${data.movie.id}`, 0)} target="_blank"><i class="fa-solid fa-play pr-1"></i>
    ...
    return `<a${add_attribute("href", `/tv/${movie.id}`, 0)}><img${add_attribute("src", media(movie.poster_path, 200), 0)}${add_attribute("alt", movie.title, 0)} class="svelte-192ei4a"> </a>`;
  })}</div></div>` : ``}`;
});
export {
  Page as default
